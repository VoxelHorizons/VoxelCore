...
public final class ItemDefinitionCompiler {
    ...
    public ItemDefinitionCompiler() {
        this(new ItemInheritanceResolver());
    }

    ItemDefinitionCompiler(ItemInheritanceResolver inheritanceResolver) {
    ...
    public ItemDefinitionRegistry compile(Collection<RawItemDefinition> rawDefinitions) {
        if (rawDefinitions == null) {
            throw new ContentCompileException("Item definitions cannot be null");
        }
        ...
        for (RawItemDefinition definition : rawDefinitions) {
            if (definition == null || definition.id() == null) {
                throw new ContentCompileException("Item definition and id cannot be null");
            }
            if (indexed.put(definition.id(), definition) != null) {
                throw new ContentCompileException("Duplicate item id: " + definition.id());
            }
        }
    ...
    public ItemDefinitionRegistry compile(Map<ContentID, RawItemDefinition> rawDefinitions) {
        ...
        for (Map.Entry<ContentID, RawItemDefinition> entry : rawDefinitions.entrySet()) {
            ...
            if (key == null || definition == null || definition.id() == null) {
                throw new ContentCompileException("Item definition keys and ids cannot be null");
            }
            if (!key.equals(definition.id())) {
                throw new ContentCompileException("Item map key " + key + " does not match definition id " + definition.id());
            }
        ...
        for (Map.Entry<ContentID, RawItemDefinition> entry : resolved.entrySet()) {
            compiled.put(entry.getKey(), compileResolved(entry.getValue()));
        }
    ...
    private ItemDefinition compileResolved(RawItemDefinition raw) {
        ...
        return new ItemDefinition(
                ...
                raw.material(),
                raw.displayName(),
                raw.lore() == null ? Collections.<String>emptyList() : raw.lore(),
                bound,
                Optional.ofNullable(raw.parent()),
                render,
    ...
    private ItemRenderDefinition compileRender(RawItemRenderDefinition raw) {
        ...
        return new ItemRenderDefinition(raw.model(), raw.legacyCustomModelData());
    }
}
