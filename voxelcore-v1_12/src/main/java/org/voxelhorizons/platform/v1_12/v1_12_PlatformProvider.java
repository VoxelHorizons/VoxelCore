package org.voxelhorizons.platform.v1_12;

import org.bukkit.plugin.Plugin;
import org.voxelhorizons.platform.PlatformProvider;
import org.voxelhorizons.platform.Version;
import org.voxelhorizons.platform.VersionAdapter;

public final class v1_12_PlatformProvider implements PlatformProvider {
    @Override
    public String name() {
        return "v1_12 (1.12-1.13.x)";
    }

    @Override
    public boolean supports(Version version) {
        return version.atLeast(1, 12, 0) && version.before(1, 14, 0);
    }

    @Override
    public VersionAdapter create(Version version, Plugin plugin) {
        return new v1_12_VersionAdapter(version);
    }
}
